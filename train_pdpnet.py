#!/usr/bin/env python3
"""
Huấn luyện PDPNet (models.ModelBT2) — dataset CIFAR hoặc ImageFolder (train/val).
Ví dụ ImageFolder (5 lớp):  data_root/
    train/class_a/*.jpg
    val/class_a/*.jpg
"""
import argparse
...
def get_args():
    parser = argparse.ArgumentParser(
        description='Train PDPNet (ModelBT2) on CIFAR or ImageFolder.',
    ...
    parser.add_argument(
        ...
        default='imagefolder',
        help='cifar10/cifar100: ảnh 32x32; imagefolder: train/ và val/ con các thư mục lớp; dogs: Stanford Dogs.',
    )
    parser.add_argument(
        '-r', '--data-root',
        type=str,
        default='./data_custom',
        help='CIFAR: thư mục chứa dữ liệu; imagefolder: thư mục cha (trong đó có train/ và val/).',
    )
    parser.add_argument('--download', action='store_true', help='(CIFAR) tải dataset nếu chưa có.')
    parser.add_argument('-g', '--gpu-id', default=0, type=int, help='GPU id, -1 = CPU.')
    parser.add_argument('-j', '--workers', default=4, type=int, help='Số worker DataLoader.')
    parser.add_argument('-b', '--batch-size', default=32, type=int, help='Batch size.')
    parser.add_argument('-e', '--epochs', default=200, type=int, help='Số epoch.')
    parser.add_argument('-l', '--learning-rate', default=0.1, type=float, help='SGD lr ban đầu.')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Đầu vào PDPNet: 32 (CIFAR) hoặc 224 (ảnh lớn). Mặc định: 32 nếu cifar*, 224 nếu imagefolder/dogs.',
    )
    parser.add_argument(
        '--n-class',
        type=int,
        default=None,
        help='Số lớp đầu ra. Bỏ trống: tự lấy từ dataset (ImageFolder: số thư mục lớp).',
    )
    parser.add_argument(
        '--run-tag',
        type=str,
        default=None,
        help='Tùy chọn: thư mục con dưới checkpoints/PDPNet_<dataset>/ để không ghi đè log (vd: D_224, D_32).',
    )
...
def build_transforms(dataset_name, train, image_size):
    ...
    # dogs / imagefolder — kích thước theo image_size (32 cho midterm, 224 cho ảnh lớn)
    ...
    if args.dataset in ('cifar10', 'cifar100'):
        ...
        if not os.path.isdir(train_dir) or not os.path.isdir(val_dir):
            raise FileNotFoundError(
                "Cần {} và {} (ImageFolder).".format(train_dir, val_dir)
            )
        ...
        if train_ds.classes != val_ds.classes:
            raise ValueError('Thứ tự / tên lớp train và val không khớp.')
        n_class = len(train_ds.classes)
        if args.n_class is not None:
            if args.n_class != n_class:
                raise ValueError('--n-class ({}) != số lớp trong data ({}).'.format(args.n_class, n_class))
...
def main():
    ...
    tag = 'PDPNet_{}'.format(args.dataset)
    ...
    for n_epoch in range(args.epochs):
        ...
        if acc_val_max is None or acc_val > acc_val_max:
            ...
            torch.save(
                {'model_state_dict': model.state_dict(), 'n_class': n_class, 'image_size': image_size},
